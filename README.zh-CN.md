...
当前默认使用 DeepSeek `deepseek-flash`，并为 DeepSeek 任务显式开启 thinking 和 `high` reasoning；Tavily 补充
网页召回，再由 Codex `gpt-5.6-luna` 以 `xhigh` reasoning 做 verifier。默认值可以按需更换，
不是硬编码限制；
